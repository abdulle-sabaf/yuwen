...
### 已完成内容：
- 《[小学现代文课文集萃](./语文/小学现代文阅读课文.pdf)》：重新遴选小学应该学习的课文，包含184篇课文，包括从历代旧教材中挑选的课文以及新选入文章。
- 《[中学现代文课文集萃](./语文/中学现代文阅读课文集萃.pdf)》：重新遴选中学应该学习的课文，包含141篇课文，包括从历代旧教材中挑选的课文以及新选入文章。
- 《[小学古诗集萃](./语文/小学古诗集萃.pdf)》：收录适合学龄前到小学阶段学习的115首古诗。以脍炙人口、朗朗上口的经典为主，辅以简单易明有童趣的佳作。我们将这115首古诗按难度分为9层。学龄前适用前三层，小学阶段适用后六层。诗歌内容全部注有拼音。推荐全部背诵。
- 《[中学古诗词集萃](./语文/中学古诗词集萃.pdf)》中学阶段学习的古诗词。共收录158首古诗词。主要搜集已经选入过课本的古诗词，少数是我们另外选入的诗歌。按难度大体分为12级，对应中学12个学期。又按重要程度分为背诵、精读、泛读三类。其中推荐背诵73首。
- 《[中学文言文阅读课文集萃](./语文/中学古文阅读课文集萃.pdf)》：中学阶段学习文言文的阅读文章。主要搜集已经选入过课本的文言文，少数是我们另外选入的文章。目前一共86篇。

### 进行的内容
- 《[学龄前发蒙识字课本](./语文/发蒙识字.pdf)》：计划为学前至小学一年级（4-7岁）的发蒙识字计划，通过约100篇分级进阶课文，掌握大约1000个基础汉字的识读和书写。不包括具体的教学材料，只有课文和识字计划，做理论上的探讨，包括基础汉字分级规划，课文与识字目标的统一等等。目前包括74课，识字约630个。
- 《国文读写新文话》：初中到高中的语文新课本。以夏丏尊和叶圣陶的《国文百八课》为基础和参考，重新打造工具性、能力培养向的语文教育计划，摆脱以范文为中心、以范文为单位的教学方式。《国文百八课》是中国语文教育史上一部颇具影响力的语文教材，由夏丏尊和叶圣陶两位语文教育家在1935年至1938年间陆续编写完成。这套教材以课为单元，原计划出版六册，共108课，由于战争原因实际只编写出版了前四册，共72课。目前尚在计划中，目标是将其重新编写并扩展到108课，覆盖初中到高中的内容。分“文话-文法-词法-修辞-逻辑”等方面内容。
...
完成品主要是前一节提到的课文集萃，包括识字课文、小学与中学的课文集萃。其中搜集了从50年代起至今编入各个版本语文教科书的大量中小学课文。识字课文包括至少70篇课文，识字量为630字左右，学完之后可以衔接小学的语文学习。小学课文包括184篇现代文，中学课文包括141篇现代文和86篇文言文。此外包括115篇初阶（学龄前和小学阶段）经典古诗以及158篇古体诗词（中学阶段）。

结构化数据则是以上内容的结构化储存版本，以`json`格式储存，编码格式为`utf-8`，可以使用`Python`、`Java`、`C`语言等进行调用。适合进行语料分析以及生成`latex`编译代码。

- [​中文发蒙识字数据.json](./src/小学/发蒙识字.json)
- [​小学现代文阅读课文数据.json](./src/小学/阅读课文.json)
- [小学古诗数据.json](./src/小学/古诗.json)
- [中学现代文阅读课文数据.json](./src/中学/阅读课文.json)
- [中学文言文阅读课文数据.json](./src/中学/中学古文阅读课文.json)
- [中学古诗词数据.json](./src/中学/古诗.json)

此外，本项目包括了从结构化数据批量生成`latex`编译代码的`Python`脚本。使用`Python`脚本可以自动化地将结构化数据中的课文转为可编译的`latex`代码，使用`Tex`编译器即可编译为可打印的`pdf`或`ps`文件。具体见[Github项目页](https://github.com/abdulle-sabaf/yuwen)。